...
if [[ -f $cfg ]]; then
  ...
  for k in idle.lock idle.screensaver; do
    v=$(jq -r "getpath(\"${k//./\",\"}\") // empty" "$cfg" 2>/dev/null)
...
# The lock plugin is the one surface that can strand a session; assert its IPC
# target exists and reports a sane state.
